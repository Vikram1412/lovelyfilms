...
$query = "SELECT RegId FROM registration WHERE 1";
if ($result=mysqli_query($conn,$query)){
	$rowcount=mysqli_num_rows($result);
	if($rowcount < 1){
		$sql = "ALTER TABLE registration AUTO_INCREMENT=10000001";
...
if($isValidChecksum == "TRUE") {
	if ($_POST["STATUS"] == "TXN_SUCCESS") {
		...
		WhoAreYou='".mysqli_real_escape_string($conn,$_SESSION['whoAreYou'])."',
		...
		$fileSource = "tmpUploads/".$_SESSION['file_name'];
		$newFileName = "IMG_".$regId;
		$fileDestination = "uploads/".$newFileName."jpg";
		copy($fileSource, $fileDestination);
		@unlink($fileSource);

		$query1 = "UPDATE registration SET ProfilePic='".mysqli_real_escape_string($conn,$newFileName)."' WHERE RegId=".$RegId."";
		...
		$mail->AddReplyTo("aftaba146@gmail.com","No Reply");
		$mail->From       = "aftaba146@gmail.com";
		...
		$mail->Send();
		header("location: statusPayment?msg=successfully!");
	}
	else {
		//echo "<b>Transaction status is failure!</b>" . "<br/>";
		header('Location: http://lovelyfilms.in/beta/lovelyfilms/register?msg=Transaction status is failure!');
	}
}
else{
